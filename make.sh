...
if [[ ! -e $romsdir/$sourcever/$romtype/$romtypename/DONTRESIGN ]]; then
    if [[ ! -e $romsdir/$sourcever/$romtype/DONTRESIGN ]]; then
        ...
        ispython3=`python -c 'import sys; print("%i" % (sys.hexversion<0x03000000))'`
        if [ $ispython3 -eq 0 ]; then
            python3=python3
        else
            python3=python
        fi
        $python3 $toolsdir/ROM_resigner/resign.py "$systemdir/system" $toolsdir/ROM_resigner/AOSP_security > $tempdir/resign.log
...
date=`date +%Y%m%d`
outputname="$romtypename-$outputtype-$sourcever-$date-TranPhong"
...
sed -i "s/$bdisplay/$displayid2=Built\.with\.ErfanGSI\.Tools\.by\.TranPhong/" $systemdir/system/build.prop
